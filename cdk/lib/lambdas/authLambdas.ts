...
export const createAuthLambdas = (stack: Stack, envVars: Record<string, string>) => {
    const  createUserLambda = createLambda({stack, envVars}, "createUserLambda", "handlers/auth/createUser.ts", "createUserHandler", ["bcryptjs", "aws-lambda", "sequelize"]);
    createUserLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses: SendEmail", "ses: SendRawEmail"], resources: ["*"]}));

    const loginLambda = createLambda({stack, envVars}, "loginLambda", "handlers/auth/login.ts", "loginHandler", ["ua-parser-js", "jsonwebtoken", "bcryptjs", "geoip-lite", "aws-lambda", "sequelize"]);
    loginLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses: SendEmail", "ses: SendRawEmail"], resources: ["*"]}));

    const getUserLambda = createLambda({stack, envVars}, "getUserLambda", "handlers/auth/getUser.ts", "getUserHandler", ["aws-lambda", "sequelize"]);
    getUserLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses: SendEmail", "ses: SendRawEmail"], resources: ["*"]}));

    const getAllUserLambda = createLambda({stack, envVars}, "getAllUserLambda", "handlers/auth/getAllUsers.ts", "getAllUsersHandler", ["aws-lambda", "sequelize"]);
    getAllUserLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses: SendEmail", "ses: SendRawEmail"], resources: ["*"]}));

    const updateUserDetailsLambda = createLambda({stack, envVars}, "updateUserDetailsLambda", "handlers/auth/updateUserDetails.ts", "updateUserDetailsHandler", ["aws-lambda", "sequelize"]);
    updateUserDetailsLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses: SendEmail", "ses: SendRawEmail"], resources: ["*"]}));

    const promoteUserLambda = createLambda({stack, envVars}, "promoteUserLambda", "handlers/auth/promoteUser.ts", "promoteUserHandler", ["aws-lambda", "sequelize"]);
    promoteUserLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses: SendEmail", "ses: SendRawEmail"], resources: ["*"]}));

    const demoteUserLambda = createLambda({stack, envVars}, "demoteUserLambda", "handlers/auth/demoteUser.ts", "demoteUserHandler", ["aws-lambda", "sequelize"]);
    demoteUserLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses: SendEmail", "ses: SendRawEmail"], resources: ["*"]}));

    const deleteUserLambda = createLambda({stack, envVars}, "deleteUserLambda", "handlers/auth/deleteUser.ts", "deleteUserHandler", ["aws-lambda", "sequelize"]);
    deleteUserLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses: SendEmail", "ses: SendRawEmail"], resources: ["*"]}));

    const resetPasswordLambda = createLambda({stack, envVars}, "resetPasswordLambda", "handlers/auth/resetPassword.ts", "resetPasswordHandler", ["sequelize", "aws-lambda"]);
    resetPasswordLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses: SendEmail", "ses: SendRawEmail"], resources: ["*"]}));

    const updatePasswordLambda = createLambda({stack, envVars}, "updatePasswordLambda", "handlers/auth/updatePassword.ts", "updatePasswordHandler", ["sequelize", "aws-lambda"]);
