...
export const createSlotLambda = (stack: Stack, envVars: Record<string, string>) => {
    const createParkingSlotLambda = createLambda({stack, envVars}, "createParkingSlotLambda", "src/handlers/parkingSlot/createParkingSlot.ts", "createParkingSlotHandler", ["aws-lambda", "sequelize"]);
    createParkingSlotLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses:SendEmail", "ses:SendRawEmail"], resources: ["*"]}));

    const getAvailableSlotLambda = createLambda({stack, envVars}, "getAvailableSlotLambda", "src/handlers/parkingSlot/fetchParkingSlot.ts", "getAvailableSlotHandler", ["aws-lambda", "sequelize"]);
    getAvailableSlotLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses:SendEmail", "ses:SendRawEmail"], resources: ["*"]}));

    const getAvailableSlotWithIdLambda = createLambda({stack, envVars}, "getAvailableSlotWithIdLambda", "src/handlers/parkingSlot/fetchWithID.ts", "getAvailableSlotWithIdHandler", ["aws-lambda", "sequelize"]);
    getAvailableSlotWithIdLambda.addToRolePolicy(new iam.PolicyStatement({actions: ["ses:SendEmail", "ses:SendRawEmail"], resources: ["*"]}));

    const updateParkingSlotLambda = createLambda({stack, envVars}, "updateParkingSlotLambda", "src/handlers/parkingSlot/updateParkingSlot.ts", "updateParkingSlotHandler", ["aws-lambda", "sequelize"]);
