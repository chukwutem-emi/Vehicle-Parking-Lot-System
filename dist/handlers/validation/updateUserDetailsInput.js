 ...
 * }
 */
export const updateUserInputValidation = (username, password, userAddress, phone, email) => {
    const usernameInput = {
        value: username,
        required: true,
        maximumLength: 100,
    ...
    if (!validation.validate(phoneInput)) {
        return {
            ...
            body: JSON.stringify({
                success: false,
                message: `Invalid phone number. Phone number is required and it must be a valid phone number with a length of ${phoneInput.minimumLength} - ${phoneInput.maximumLength} digits. Please ensure your phone number meets these requirements.`
            })
        };
    }
    ;
